...
function Calendar(element, options, eventSources) {
	...
	function reportEvents(_events, _annotations) {
		events = _events;
        if( _annotations ) {
...
function EventManager(options, _sources) {
	...
	function fetchEventSource(source, fetchID) {
		_fetchEventSource(source, function(events, annotations) {
			if (fetchID == currentFetchID) {
				...
				pendingSourceCnt--;
				if (!pendingSourceCnt) {
					reportEvents(cache, annotations);
				}
			}
		});
	}
	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			if ($.isFunction(events)) {
				pushLoading();
				events(cloneDate(rangeStart), cloneDate(rangeEnd), function(events, annotations) {
					callback(events, annotations);
					popLoading();
				});
			}
